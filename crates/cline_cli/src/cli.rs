//! The [`Cline`] executor and the crate [`Error`] type.
...
/// Errors returned while running Cline.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum Error {
    /// Cline could not be started or its output could not be collected.
    #[error("cline failed: {0}")]
    Io(#[from] std::io::Error),
    /// Cline exited unsuccessfully without usable machine-readable output.
    #[error("cline exited with code {exit_code:?}: {stderr}")]
    Cli {
        /// Cline exit code.
    ...
    /// Cline emitted malformed JSON for the requested output format.
    #[error("failed to parse cline JSON output: {0}")]
    Json(#[from] serde_json::Error),
}

/// Incremental reader for `cline --json`.
#[derive(Debug)]
struct EventReader {
    child: Child,
...
impl EventReader {
    ...
    /// lines. Returns `Ok(None)` after Cline closes stdout.
    ...
    /// Drain remaining stdout and wait for Cline to exit successfully.
...
/// A Cline command that can be executed to completion.
#[doc(hidden)]
...
/// Cline executable context.
///
/// This owns the Cline executable path, working directory, and environment
...
impl Cline {
    /// Build an executor that invokes a custom Cline executable.
    #[must_use]
    ...
    /// Cline executable name or path.
    #[must_use]
    pub fn executable(&self) -> &OsStr {
        &self.executable
    }

    /// Working directory used when Cline runs.
    #[must_use]
    pub fn current_dir(&self) -> Option<&Path> {
        self.current_dir.as_deref()
    }

    /// Environment overrides used when Cline runs.
    ...
    /// Set the working directory used when Cline runs.
    ...
    /// Add or replace an environment variable used when Cline runs.
    ...
    /// Convert a typed Cline command into a process builder.
    ///
    ...
    /// Execute a Cline command to completion.
    ///
    /// # Errors
    ///
    /// Returns [`Error::Io`] when Cline cannot run or output cannot be
    ...
    /// Start `cline --json` and read its events incrementally.
    ///
    ...
    /// # Errors
    ///
    /// Returns [`Error::Io`] when Cline cannot be started.
    pub fn stream(&self, command: &JsonRunCommand) -> Result<EventStream, Error> {
        ...
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| Error::Io(std::io::Error::other("cline stream stdout was unavailable")))?;
