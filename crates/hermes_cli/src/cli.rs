//! The [`Hermes`] executor and the crate [`Error`] type.
...
/// Errors returned while running Hermes.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum Error {
    /// `hermes` could not be started or its output could not be collected.
    #[error("hermes failed: {0}")]
    Io(#[from] std::io::Error),
    /// `hermes` exited unsuccessfully. Because the agent path is text-only and
    /// its exit code is overloaded, this is produced only on demand via
    /// [`RunOutput::into_result`](crate::RunOutput::into_result), never by
    /// [`Hermes::execute`].
    #[error("hermes exited with code {exit_code:?}: {stderr}")]
    Cli {
        /// `hermes` exit code, when one was produced.
    ...
    /// A structured surface (`send --json` / `sessions export`) produced output
    /// that could not be parsed as the expected JSON shape.
    #[error("hermes produced malformed JSON output: {0}")]
    Json(#[from] serde_json::Error),
}

/// Hermes executable context.
///
/// This owns the `hermes` executable path, working directory, and environment
/// overrides. Command semantics live in command values (`RunCommand`,
/// `ChatCommand`, `SendCommand`, …).
#[derive(Debug, Clone)]
pub struct Hermes {
    executable: OsString,
...
impl Hermes {
    /// Build an executor that invokes a custom `hermes` executable.
    #[must_use]
    ...
    /// `hermes` executable name or path.
    #[must_use]
    ...
    /// Working directory used when `hermes` runs.
    #[must_use]
    pub fn current_dir(&self) -> Option<&Path> {
        self.current_dir.as_deref()
    }

    /// Environment overrides used when `hermes` runs.
    ...
    /// Set the working directory used when `hermes` runs.
    ...
    /// Add or replace an environment variable used when `hermes` runs.
    ...
    /// Convert a typed Hermes command into a process builder.
    ///
    ...
    /// Execute a Hermes command to completion.
    ///
    /// Returns the decoded stdout/stderr and a typed [`Exit`](crate::Exit)
    /// classification. A non-zero exit is *not* an error here: Hermes overloads
    ...
    /// # Errors
    ///
    /// Returns [`Error::Io`] when `hermes` cannot run or its output cannot be
    /// collected.
