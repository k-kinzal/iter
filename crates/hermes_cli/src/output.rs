//! Text-path output for `hermes`.
//!
//! Hermes' agent path (`-z` oneshot and `chat -Q`) has no machine-readable
//! output, so a run's "output" is its decoded stdout/stderr plus a typed
//! [`Exit`] classification. There is no per-turn event stream: higher-level
//! meaning (token limits, tracebacks, argparse rejections) is scanned from the
//! text by the caller.
//!
//! The two structured surfaces — `send --json` and `sessions export` — are
//! modeled separately in [`send`](crate::send) and
...
/// The disposition of a finished `hermes` process.
///
/// Hermes overloads its exit code (see the crate README), so this classifies
/// the raw disposition only; it does not decide success/failure meaning.
...
/// The complete output of a finished `hermes` run on the text path.
...
impl RunOutput {
    ...
    ///
    /// Hermes' scripted exit code is overloaded, so callers that need the text
    /// even on a non-zero exit should inspect [`RunOutput`] directly rather
    /// than through this convenience.
    ///
    /// # Errors
    ///
