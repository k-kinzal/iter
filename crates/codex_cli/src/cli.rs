//! The [`Codex`] executor and the crate [`Error`] type.
...
use crate::exec::JsonExecCommand;
use crate::output::{Event, EventStream};

/// Errors returned while running Codex.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum Error {
    /// Codex could not be started or its output could not be collected.
    #[error("codex failed: {0}")]
    Io(#[from] std::io::Error),
    /// Codex exited unsuccessfully without usable machine-readable output.
    #[error("codex exited with code {exit_code:?}: {stderr}")]
    Cli {
        /// Codex exit code.
    ...
    /// Codex emitted malformed JSON for the requested output format.
    #[error("failed to parse codex JSON output: {0}")]
    Json(#[from] serde_json::Error),
}

/// Incremental reader for `codex exec --json`.
#[derive(Debug)]
struct EventReader {
    child: Child,
...
impl EventReader {
    ...
    /// lines. Returns `Ok(None)` after Codex closes stdout.
    ...
    /// Drain remaining stdout and wait for Codex to exit successfully.
...
/// A Codex command that can be executed to completion.
#[doc(hidden)]
...
/// Codex executable context.
///
/// This owns the Codex executable path, working directory, and environment
...
impl Codex {
    /// Build an executor that invokes a custom Codex executable.
    #[must_use]
    ...
    /// Working directory used when Codex runs.
    #[must_use]
    pub fn current_dir(&self) -> Option<&Path> {
        self.current_dir.as_deref()
    }

    /// Environment overrides used when Codex runs.
    ...
    /// Set the working directory used when Codex runs.
    ...
    /// Add or replace an environment variable used when Codex runs.
    ...
    /// Convert a typed Codex command into a process builder.
    ///
    ...
    /// Execute a Codex command to completion.
    ///
    /// # Errors
    ///
    /// Returns [`Error::Io`] when Codex cannot run or output cannot be
    ...
    /// Start `codex exec --json` and read its events incrementally.
    ///
    ...
    /// # Errors
    ///
    /// Returns [`Error::Io`] when Codex cannot be started.
    pub fn stream(&self, command: &JsonExecCommand) -> Result<EventStream, Error> {
        ...
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| Error::Io(std::io::Error::other("codex stream stdout was unavailable")))?;
...
impl ExecutableCommand for JsonExecCommand {
    type Output = crate::output::ExecOutput;
}
