//! The [`Cursor`] executor and the crate [`Error`] type.
...
use crate::run::PrintCommand;

/// Errors returned while running `cursor-agent`.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum Error {
    /// `cursor-agent` could not be started or its output could not be
    /// collected.
    #[error("cursor-agent failed: {0}")]
    Io(#[from] std::io::Error),
    /// `cursor-agent` exited unsuccessfully without usable machine-readable
    /// output.
    #[error("cursor-agent exited with code {exit_code:?}: {stderr}")]
    Cli {
    ...
    /// `cursor-agent` emitted malformed JSON for the requested output format.
    #[error("failed to parse cursor-agent JSON output: {0}")]
    Json(#[from] serde_json::Error),
}

/// Incremental reader for `cursor-agent --print --output-format stream-json`.
#[derive(Debug)]
struct EventReader {
    child: Child,
...
impl EventReader {
    ...
    /// lines. Returns `Ok(None)` after `cursor-agent` closes stdout.
    ...
    /// Drain remaining stdout and wait for `cursor-agent` to exit successfully.
...
/// A `cursor-agent` command that can be executed to completion.
#[doc(hidden)]
...
/// `cursor-agent` executable context.
///
/// This owns the `cursor-agent` executable path, working directory, and
/// environment overrides. Command semantics and output type live in command
/// values.
#[derive(Debug, Clone)]
pub struct Cursor {
    executable: OsString,
...
impl Cursor {
    /// Build an executor that invokes a custom `cursor-agent` executable.
    #[must_use]
    ...
    /// `cursor-agent` executable name or path.
    #[must_use]
    ...
    /// Working directory used when `cursor-agent` runs.
    #[must_use]
    pub fn current_dir(&self) -> Option<&Path> {
        self.current_dir.as_deref()
    }

    /// Environment overrides used when `cursor-agent` runs.
    ...
    /// Set the working directory used when `cursor-agent` runs.
    ...
    /// Add or replace an environment variable used when `cursor-agent` runs.
    ...
    /// Convert a typed `cursor-agent` command into a process builder.
    ///
    ...
    /// handling, cancellation, stdin delivery, or stdout/stderr teeing.
    ...
    /// Execute a `cursor-agent` command to completion.
    ///
    /// The prompt (if any) is delivered as the command's positional argument
    /// and stdin is closed; callers needing prompt-over-stdin should build the
    /// process with [`to_process`](Cursor::to_process) and drive it directly.
    ///
    /// # Errors
    ///
    /// Returns [`Error::Io`] when `cursor-agent` cannot run or output cannot be
    ...
    /// Start `cursor-agent --print --output-format stream-json` and read its
    /// events incrementally.
    ///
    /// Events are returned as an [`EventStream`]. The stream also checks the
    /// final process status after stdout closes. The prompt is delivered as
    /// the command's positional argument and stdin is closed.
    ///
    /// # Errors
    ///
    /// Returns [`Error::Io`] when `cursor-agent` cannot be started.
    pub fn stream(&self, command: &PrintCommand) -> Result<EventStream, Error> {
        ...
        let stdout = child.stdout.take().ok_or_else(|| {
            Error::Io(std::io::Error::other(
                "cursor-agent stream stdout was unavailable",
            ))
        })?;
...
impl ExecutableCommand for PrintCommand {
    type Output = crate::output::PrintOutput;
}
