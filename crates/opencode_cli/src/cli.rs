//! The [`Opencode`] executor and the crate [`Error`] type.
...
/// Errors returned while running opencode.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum Error {
    /// opencode could not be started or its output could not be collected.
    #[error("opencode failed: {0}")]
    Io(#[from] std::io::Error),
    /// opencode exited unsuccessfully without usable machine-readable output.
    #[error("opencode exited with code {exit_code:?}: {stderr}")]
    Cli {
        /// opencode exit code.
    ...
    /// opencode emitted malformed JSON for the requested output format.
    #[error("failed to parse opencode JSON output: {0}")]
    Json(#[from] serde_json::Error),
}

/// Incremental reader for `opencode run --format json`.
#[derive(Debug)]
struct EventReader {
    child: Child,
...
impl EventReader {
    ...
    /// lines. Returns `Ok(None)` after opencode closes stdout.
    ...
    /// Drain remaining stdout and wait for opencode to exit successfully.
...
/// An opencode command that can be executed to completion.
#[doc(hidden)]
...
/// opencode executable context.
///
/// This owns the opencode executable path, working directory, and environment
...
impl Opencode {
    /// Build an executor that invokes a custom opencode executable.
    #[must_use]
    ...
    /// Environment overrides used when opencode runs.
    ...
    /// Set the working directory used when opencode runs.
    ...
    /// Add or replace an environment variable used when opencode runs.
    ...
    /// Convert a typed opencode command into a process builder.
    ///
    ...
    /// Execute an opencode command to completion.
    ///
    /// # Errors
    ///
    /// Returns [`Error::Io`] when opencode cannot run or output cannot be
    ...
    /// Start `opencode run --format json` and read its events incrementally.
    ///
    ...
    /// # Errors
    ///
    /// Returns [`Error::Io`] when opencode cannot be started.
