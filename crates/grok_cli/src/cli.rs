//! The [`Grok`] executor and the crate [`Error`] type.
...
use crate::single::{JsonSingleCommand, StreamingSingleCommand};

/// Errors returned while running Grok.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum Error {
    /// Grok could not be started or its output could not be collected.
    #[error("grok failed: {0}")]
    Io(#[from] std::io::Error),
    /// Grok exited unsuccessfully without usable machine-readable output.
    #[error("grok exited with code {exit_code:?}: {stderr}")]
    Cli {
        /// Grok exit code.
    ...
    /// Grok emitted malformed JSON for the requested output format.
    #[error("failed to parse grok JSON output: {0}")]
    Json(#[from] serde_json::Error),
}

/// Incremental reader for `grok … --output-format streaming-json`.
#[derive(Debug)]
struct EventReader {
    child: Child,
...
impl EventReader {
    ...
    /// lines. Returns `Ok(None)` after Grok closes stdout.
    ...
    /// Drain remaining stdout and wait for Grok to exit successfully.
...
/// A Grok command that can be executed to completion.
#[doc(hidden)]
...
/// Grok executable context.
///
/// This owns the Grok executable path, working directory, and environment
...
impl Grok {
    /// Build an executor that invokes a custom Grok executable.
    #[must_use]
    ...
    /// Working directory used when Grok runs.
    #[must_use]
    pub fn current_dir(&self) -> Option<&Path> {
        self.current_dir.as_deref()
    }

    /// Environment overrides used when Grok runs.
    ...
    /// Set the working directory used when Grok runs.
    ...
    /// Add or replace an environment variable used when Grok runs.
    ...
    /// Convert a typed Grok command into a process builder.
    ///
    ...
    /// Execute a Grok command to completion.
    ///
    /// # Errors
    ///
    /// Returns [`Error::Io`] when Grok cannot run or output cannot be collected.
    ...
    /// Start `grok … --output-format streaming-json` and read its events
    /// incrementally.
    ///
    ...
    /// # Errors
    ///
    /// Returns [`Error::Io`] when Grok cannot be started.
    pub fn stream(&self, command: &StreamingSingleCommand) -> Result<EventStream, Error> {
        ...
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| Error::Io(std::io::Error::other("grok stream stdout was unavailable")))?;
...
impl ExecutableCommand for JsonSingleCommand {
    type Output = crate::output::SingleOutput;
}
