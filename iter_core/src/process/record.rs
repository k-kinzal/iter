...
impl ProcessRecord {
    ...
    #[must_use]
    pub fn new(paths: Arc<ProcPaths>) -> Self {
        Self { paths }
    }
    ...
    pub fn open(root: &Path, id: ProcessId) -> Result<Self> {
    ...
    ///
    /// Returns an error if the operation fails.
    pub fn iterfile(&self) -> Result<PathBuf> {
    ...
    ///
    /// Returns an error if the operation fails.
    pub fn subcommand(&self) -> Result<String> {
    ...
    pub fn metadata(&self) -> Result<ProcessMetadata> {
    ...
    pub fn read_status_token(&self) -> Result<ProcessStatus> {
    ...
    /// unrecognised tokens.
    pub fn read_status_token_or_in_transition(&self) -> Result<Option<ProcessStatus>> {
    ...
    #[must_use]
    pub fn pid_identity(&self) -> PidFileState {
...
fn parse_id_from_dir(dir: &Path) -> Result<ProcessId> {
...
/// record.
pub fn list_default() -> Result<Vec<ProcessRecord>> {
...
///
/// Returns an error if the operation fails.
pub fn list_under(root: &Path) -> Result<Vec<ProcessRecord>> {
    let mut out = Vec::new();
