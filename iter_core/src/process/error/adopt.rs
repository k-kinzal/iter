...
pub enum AdoptError {
    ...
    /// `bootstrap_token` file is missing; this id has already been adopted.
    AlreadyAdopted,
    /// `bootstrap_token` was present but did not match the value the parent
    /// passed via the well-known file.
    TokenMismatch,
    /// `bootstrap_token` failed validation (length / hex / case).
...
impl fmt::Display for AdoptError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AdoptError::TokenMismatch => f.write_str("bootstrap token mismatch"),
            AdoptError::CorruptToken(kind) => write!(f, "corrupt bootstrap token: {kind:?}"),
