...
pub use sender::{LogSender, global_log_sender, install_global_log_sender};
pub use sink::{DEFAULT_LOG_BUFFER, ProcessLogSink};
...
pub async fn open_output(policy: &OutputPolicy) -> io::Result<ProcessOutput> {
    match policy {
...
mod tests {
    ...
    async fn open_output_log_only_opens_files_and_returns_sender() {
        ...
        let entries: Vec<crate::log::LogEntry> = body
            .lines()
