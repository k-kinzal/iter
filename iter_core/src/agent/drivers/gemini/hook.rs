...
//! [`GeminiDriver`](crate::agent::GeminiDriver)'s interactive/TUI mode.
//!
...
fn hook_script_body(user_hooks_script: Option<&Path>) -> String {
    ...
    let mut script = String::from(
        "#!/usr/bin/env bash\n\
         # iter AfterAgent hook for Gemini — installed by iter_core::agent::GeminiDriver.\n\
         #\n\
...
impl HookBundle {
    ...
    pub(crate) async fn install(cwd: &Path, isolation_key: &str) -> Result<Self, AgentError> {
        ...
        let user_hooks_script =
            extract_user_hooks(&settings_path, "AfterAgent", &hooks_dir, &hook_cmd).await?;
