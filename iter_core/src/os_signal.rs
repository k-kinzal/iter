//! OS-signal cancellation helpers.
//!
//! This module is the neutral home for mirroring process-level interrupts
//! (`SIGINT` / `SIGTERM`) onto a [`CancellationToken`]. It is intentionally
//! separate from the work-item [`signal`](crate::signal) module and from the
//! CLI-owned iter process record layer.
...
///
/// ```ignore
/// let cancel = iter_core::os_signal::install_signal_handlers(CancellationToken::new())?;
...
/// [`install_signal_handlers`] and CLI-side run-record reason classification
/// build on it.
///
