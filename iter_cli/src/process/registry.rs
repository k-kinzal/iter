...
//!
//! ```text
//! register(name) -> (Arc<ProcessSession>, LockGuard, [AdoptionToken])
//! ```
//!
//! Foreground and detached share the locking + id-allocation + session-init
//! sequence; detached additionally writes the `adoption_token`.
//!
...
use crate::process::adoption_token;
use crate::process::error::{ProcessError, RegistryError};
use crate::process::id::{AdoptionToken, ProcessId};
...
impl ProcessRegistry {
    ...
    pub(crate) fn open_default() -> Result<Self, ProcessError> {
    ...
    pub(crate) fn open(proc_root: impl Into<PathBuf>) -> Result<Self, ProcessError> {
    ...
    #[must_use]
    pub(crate) fn proc_root(&self) -> &Path {
        &self.proc_root
    }
    ...
    /// it.
    pub(crate) async fn register_foreground(
        &self,
        name: &str,
        draft: MetadataDraft,
    ...
    ///
    /// Returns an error if the operation fails.
    /// adoption token at `<dir>/adoption_token` (mode `0o600`,
    /// `O_CREAT|O_EXCL`).
    ///
    /// The returned `AdoptionToken` is what the parent advertises — the
    /// child re-reads the same value from disk during adoption.
    pub(crate) async fn register_detached(
        &self,
        name: &str,
        draft: MetadataDraft,
    ) -> Result<(Arc<ProcessSession>, LockGuard, AdoptionToken), RegisterError> {
        let id = ProcessId::generate();
        let lock = self.acquire(name, id).await?;
        let session = match self.create_session(name, id, draft).await {
            Ok(s) => s,
        ...
        if let Err(io) = adoption_token::write_excl(session.paths().dirfd(), &token) {
    ...
    /// Returns an error if the operation fails.
    /// other dot-directory).
    pub(crate) fn list(&self) -> Result<Vec<ProcessRecord>, ProcessError> {
    ...
    ///
    /// Returns an error if the operation fails.
    pub(crate) fn get(&self, id: ProcessId) -> Result<ProcessRecord, ProcessError> {
    ...
    pub(crate) fn find_by_name(&self, name: &str) -> Result<Option<ProcessRecord>, ProcessError> {
...
pub(crate) enum RegisterError {
    ...
    /// Failure inside session creation or adoption-token publication.
...
mod tests {
    ...
    #[tokio::test]
    async fn register_detached_publishes_adoption_token() {
        let tmp = TempDir::new().unwrap();
        let registry = ProcessRegistry::open(tmp.path()).expect("open");
        ...
        let on_disk = adoption_token::read(session.paths().dirfd()).expect("read token");
