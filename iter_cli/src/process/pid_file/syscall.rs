...
pub(super) fn write_all(fd: &OwnedFd, mut buf: &[u8]) -> io::Result<()> {
    while !buf.is_empty() {
        // SAFETY: `fd` is an owned live file descriptor, and `buf.as_ptr()`
        // is valid for `buf.len()` bytes for the duration of `write`.
...
pub(super) fn fsync_fd(fd: &OwnedFd) -> io::Result<()> {
    // SAFETY: `fd.as_raw_fd()` is a live file descriptor owned by `fd`; fsync
    // has no additional memory-safety preconditions.
...
pub(super) fn fsync_dirfd(dirfd: BorrowedFd<'_>) -> io::Result<()> {
    // SAFETY: `dirfd.as_raw_fd()` is a live borrowed directory descriptor;
    // fsync has no additional memory-safety preconditions.
...
pub(super) fn unlinkat_name(dirfd: BorrowedFd<'_>, name: &str) -> io::Result<()> {
    ...
    // SAFETY: `dirfd` is live and `cstr` is NUL-terminated and valid for the
