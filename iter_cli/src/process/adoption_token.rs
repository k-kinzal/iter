//! Read/write of `<dir>/adoption_token`, the anti-accidental-adoption file
...
use crate::process::id::AdoptionToken;
...
/// Read `<dirfd>/adoption_token` and parse it into an [`AdoptionToken`].
///
/// Validation rules (per plan D4):
/// - Length must be exactly 32 hex bytes (16 bytes encoded).
/// - Lower-case ASCII hex only.
#[cfg(unix)]
pub(crate) fn read(dirfd: BorrowedFd<'_>) -> Result<AdoptionToken, TokenReadError> {
    let name = std::ffi::CString::new(names::ADOPTION_TOKEN).expect("static name has no NUL");
    // SAFETY: `dirfd` is a live directory fd supplied by `ProcPaths`; `name`
    // is a NUL-terminated static file name and remains valid for the call.
    let fd = unsafe {
        libc::openat(
            dirfd.as_raw_fd(),
            name.as_ptr(),
    ...
    // owned anywhere else, so transferring ownership to `OwnedFd` is correct.
...
#[cfg(not(unix))]
pub(crate) fn read(_dirfd: BorrowedFd<'_>) -> Result<AdoptionToken, TokenReadError> {
    Err(TokenReadError::Io(io::Error::new(
        io::ErrorKind::Unsupported,
        "adoption_token::read is unix-only",
    )))
}

fn parse(hex: &str) -> Result<AdoptionToken, TokenCorruptKind> {
    if hex.len() != 32 {
    ...
    Ok(AdoptionToken::from_bytes(bytes))
}

/// Create `<dirfd>/adoption_token` exclusively (`O_CREAT|O_EXCL|0600`) and
/// write the token's hex body. Existing file → `EEXIST` propagates.
#[cfg(unix)]
pub(crate) fn write_excl(dirfd: BorrowedFd<'_>, token: &AdoptionToken) -> io::Result<()> {
    let name = std::ffi::CString::new(names::ADOPTION_TOKEN).expect("static name has no NUL");
    // SAFETY: `dirfd` is a live directory fd supplied by `ProcPaths`; `name`
    // is a NUL-terminated static file name and `FILE_MODE` is a valid mode.
    let fd = unsafe {
        libc::openat(
            dirfd.as_raw_fd(),
            name.as_ptr(),
...
#[cfg(not(unix))]
pub(crate) fn write_excl(_dirfd: BorrowedFd<'_>, _token: &AdoptionToken) -> io::Result<()> {
    Err(io::Error::new(
        io::ErrorKind::Unsupported,
        "adoption_token::write_excl is unix-only",
    ))
}

/// Best-effort delete of `<dirfd>/adoption_token`. ENOENT is **not** an
/// error (the file may already have been deleted).
#[cfg(unix)]
pub(crate) fn delete(dirfd: BorrowedFd<'_>) -> io::Result<()> {
    let name = std::ffi::CString::new(names::ADOPTION_TOKEN).expect("static name has no NUL");
    // SAFETY: `dirfd` is a live directory fd and `name` is a valid
    // NUL-terminated file name for the duration of the syscall.
...
#[cfg(not(unix))]
pub(crate) fn delete(_dirfd: BorrowedFd<'_>) -> io::Result<()> {
    Ok(())
}

/// Predicate used by tests to assert the adoption token has (or has
...
pub(crate) fn exists(dirfd: BorrowedFd<'_>) -> bool {
    ...
    // SAFETY: `dirfd` is a live directory fd, `name` is NUL-terminated, and
    // `st` points to writable storage for the duration of the call.
    let r = unsafe {
        libc::fstatat(
            dirfd.as_raw_fd(),
            name.as_ptr(),
...
mod tests {
    ...
    fn write_then_read_round_trip() {
        ...
        let token = AdoptionToken::generate();
