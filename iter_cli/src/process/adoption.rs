...
//! `meta.json` + `status=initializing` + `adoption_token`, and exec'd into
...
//!    on Linux).
//! 3. Read `adoption_token` from disk so [`locked_adoption_write`] has a
...
use crate::process::adoption_token::{self, TokenReadError};
...
pub(crate) async fn adopt_from_argv(
    root: &Path,
    id: ProcessId,
...
async fn try_adopt(
    ...
    let expected = match adoption_token::read(session.paths().dirfd()) {
        Ok(t) => t,
...
mod tests {
    ...
    async fn adopt_from_argv_publishes_pid_and_flips_to_running() {
        ...
        // pid file was published; adoption_token was deleted.
        let dir = tmp.path().join(id.to_string());
        assert!(dir.join("pid").exists(), "pid file must exist");
        assert!(
            !dir.join("adoption_token").exists(),
            "adoption_token must be deleted after adoption"
        );
    }
    ...
    async fn adopt_from_argv_returns_already_adopted_when_token_missing() {
        ...
        let token_path = tmp.path().join(id.to_string()).join("adoption_token");
