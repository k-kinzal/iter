...
//! `adoption_token`, and acquires the name lock) with a `fork+setsid+exec`
//! using [`std::process::Command`]. The child receives only the ULID via
//! `--process-id <ULID>`; the adoption token travels via the filesystem
...
pub(crate) enum SpawnError {
    ...
    /// session directory I/O, adoption token publication).
...
pub(crate) struct UnmanagedChild {
...
#[cfg(unix)]
pub(crate) fn spawn_unmanaged_detached(
...
mod tests {
    ...
    async fn spawn_detached_creates_record_and_runs_child() {
        ...
        // adoption_token survives until adoption deletes it; `true`
        // never adopts, so it is still on disk.
        assert!(dir.join("adoption_token").exists());
    }
    ...
    async fn spawn_detached_appends_process_id_argv() {
        ...
        let script = tmp.path().join("argv.sh");
        ...
        .expect("write argv script");

        let mut spec = sample_spec("argv", PathBuf::from("/bin/sh"));
        spec.args = vec![script.display().to_string(), "one".into(), "two".into()];

        let id = spawn_detached(&registry, spec)
            .await
            .expect("spawn detached");
        let id_string = id.to_string();
        let expected = vec!["one", "two", "--process-id", id_string.as_str()];
