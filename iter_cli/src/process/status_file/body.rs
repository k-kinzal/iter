...
use crate::process::error::SecondaryStatusWrite;
...
/// the underlying I/O results into a [`SecondaryStatusWrite`] so the
/// caller can report exactly which step survived.
pub(super) fn best_effort_mark_failed(file: &mut File) -> SecondaryStatusWrite {
    let write_result = write_status_in_place(file, ProcessStatus::Failed);
    let fsync_result = fsync_with_one_retry(file);
    SecondaryStatusWrite::from_write_and_fsync(write_result, fsync_result)
}
