...
impl ProcessStatusFile {
    ...
    pub(crate) async fn locked_initial_write(
        self: Arc<Self>,
    ...
    /// token (file-based, see `process::adoption_token`) and best-effort
    /// deletes it after the Running flip.
    pub(crate) async fn locked_adoption_write(
        self: Arc<Self>,
        identity: ProcessIdentity,
        paths: Arc<ProcPaths>,
        expected_token: AdoptionToken,
    ) -> Result<(), AdoptError> {
        let join = spawn_blocking(move || {
            self.with_locked_status(|file| -> Result<(), AdoptError> {
                match read_status(file) {
                    ...
                    // to Running and (typically) the adoption_token was
                ...
                match adoption_token::read(paths.dirfd()) {
                    Ok(stored) if stored == expected_token => {}
                    Ok(_) => return Err(AdoptError::TokenMismatch),
                    Err(adoption_token::TokenReadError::NotFound) => {
                        return Err(AdoptError::AlreadyAdopted);
                    }
                    Err(adoption_token::TokenReadError::Corrupt(k)) => {
                        return Err(AdoptError::CorruptToken(k));
                    }
                    Err(adoption_token::TokenReadError::Io(e)) => {
                ...
                // adoption_token cleanup: best-effort; refresh sweeps.
                if let Err(io) = adoption_token::delete(paths.dirfd()) {
                    tracing::warn!(
                        error = %io,
                        "adoption_token unlink failed after adoption; will be cleaned by refresh"
                    );
                }
                Ok(())
            })
...
/// held; the resulting [`SecondaryStatusWrite`] travels back inside
...
mod tests {
    ...
    async fn locked_adoption_write_flips_running_and_deletes_token() {
        ...
        adoption_token::write_excl(paths.dirfd(), &token).expect("write token");
        sf.clone()
        ...
        let observed = sf.read_status().await.expect("read");
        assert_eq!(observed, ProcessStatus::Running);
        assert!(!adoption_token::exists(paths.dirfd()));
    }
    ...
    async fn locked_adoption_write_token_mismatch_rejects() {
        ...
        let sf = ProcessStatusFile::create_initial_locked(paths.clone())
            .await
            .unwrap();
        let real = AdoptionToken::generate();
        adoption_token::write_excl(paths.dirfd(), &real).expect("write");
        let other = AdoptionToken::generate();
        let err = sf
            .clone()
    ...
    async fn locked_adoption_write_rejects_terminal_status() {
        ...
        let token = AdoptionToken::generate();
        adoption_token::write_excl(paths.dirfd(), &token).unwrap();
        let err = sf
            .clone()
            .locked_adoption_write(fake_identity(), paths.clone(), token)
            .await
