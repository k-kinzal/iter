...
use crate::process::status::{self, ProcessStatus, StatusTransition};
...
#[derive(Debug)]
pub(crate) struct ProcessStatusFile {
...
impl ProcessStatusFile {
    ...
    pub(crate) async fn create_initial_locked(
        paths: Arc<ProcPaths>,
    ...
    pub(crate) async fn open_for_existing(
        paths: Arc<ProcPaths>,
    ) -> Result<Arc<Self>, ProcessError> {
        spawn_blocking(move || {
            let path = paths.join(names::STATUS);
    ...
    pub(crate) async fn transition(
        ...
        spawn_blocking(move || {
            self.with_locked_status(|file| -> Result<StatusTransition, ProcessError> {
    ...
    pub(crate) async fn read_status(self: Arc<Self>) -> Result<ProcessStatus, ProcessError> {
        spawn_blocking(move || {
    ...
    /// transitioned again; cleanup of leftover `adoption_token` /
    ...
    pub(crate) async fn reconcile_under_lock(
        self: Arc<Self>,
        paths: Arc<ProcPaths>,
...
mod tests {
    ...
    async fn reconcile_initializing_grace_elapsed_with_pid_marks_failed_and_cleans_token() {
        ...
        let sf = ProcessStatusFile::create_initial_locked(paths.clone())
            .await
            .unwrap();
        // Drop a synthetic pid file + adoption_token to simulate stuck-init.
        ...
        let observed = sf
            ...
            .expect("reconcile");
        assert_eq!(observed, ProcessStatus::Failed);
        assert!(!paths.join(names::ADOPTION_TOKEN).exists());
    }
