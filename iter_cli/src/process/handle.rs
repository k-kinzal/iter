...
pub(crate) const BOOTSTRAP_GRACE_ENV: &str = "ITER_PROCESS_BOOTSTRAP_GRACE_SECS";
...
#[must_use]
pub(crate) fn bootstrap_grace() -> Duration {
...
impl ProcessHandle {
    ...
    pub(crate) async fn open(root: &Path, id: ProcessId) -> ProcessFallible<Self> {
    ...
    #[must_use]
    pub(crate) fn from_session(session: &Arc<ProcessSession>) -> Self {
    ...
    #[must_use]
    pub(crate) fn record(&self) -> &ProcessRecord {
        &self.record
    }
    ...
    ///
    /// Returns an error if the operation fails.
    pub(crate) async fn status(&self) -> ProcessFallible<ProcessStatus> {
    ...
    pub(crate) async fn refresh_status(&self) -> ProcessFallible<ProcessStatus> {
    ...
    /// "already stopped".
    pub(crate) async fn stop(&self) -> ProcessFallible<StatusTransition> {
    ...
    ///
    /// Returns an error if the operation fails.
    pub(crate) async fn kill(&self) -> ProcessFallible<StatusTransition> {
    ...
    #[cfg(unix)]
    pub(crate) fn force_kill(&self) -> ProcessFallible<bool> {
    ...
    pub(crate) async fn remove(&self) -> ProcessFallible<()> {
        ...
        tokio::task::spawn_blocking(move || -> ProcessFallible<()> {
    ...
    async fn signal_and_kill(&self, kind: PosixSignal) -> ProcessFallible<StatusTransition> {
...
fn release_lock_entry(proc_root: &Path, name: &str, expected_id: ProcessId) -> ProcessFallible<()> {
