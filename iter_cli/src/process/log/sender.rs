...
use iter_core::log::{LogEntry, LogStream, WriterErrorSlot, WriterMsg, writer_dead_error};
...
impl LogSender {
    ...
    pub(crate) fn try_send_line(&self, stream: LogStream, line: String) {
    ...
    pub(crate) async fn send_line(&self, stream: LogStream, line: String) -> io::Result<()> {
        self.sender
...
pub(crate) fn install_global_log_sender(sender: LogSender) {
    drop(GLOBAL_LOG_SENDER.set(sender));
}

/// Borrow the process-wide [`LogSender`] when one has been installed.
#[must_use]
pub(crate) fn global_log_sender() -> Option<&'static LogSender> {
