...
/// Mode for every per-id file (`pid`, `status`, `adoption_token`, ...).
...
pub(crate) mod names {
    ...
    pub(crate) const PID_TMP: &str = ".pid.tmp";
    ...
    pub(crate) const ADOPTION_TOKEN: &str = "adoption_token";
    /// Plain-text registered name.
    pub(crate) const NAME: &str = "name";
    /// Absolute path of the `Iterfile` that was loaded.
    pub(crate) const ITERFILE: &str = "iterfile";
    ...
    pub(crate) const LOG_NDJSON: &str = "log.ndjson";
    /// Per-record lifetime lock. The running process holds an exclusive
    /// `flock` on this file from session creation/adoption until exit.
    pub(crate) const LIFETIME_LOCK: &str = "lifetime.lock";
}
...
pub(crate) fn proc_root_default() -> Result<PathBuf, ProcessError> {
    let home = iter_core::home::home_dir().ok_or_else(|| {
...
impl ProcPaths {
    ...
    #[must_use]
    pub(crate) fn from_parts(id: ProcessId, dir: PathBuf, dirfd: OwnedFd) -> Self {
        Self { id, dir, dirfd }
    }
    ...
    pub(crate) fn create_for_new_id(root: &Path, id: ProcessId) -> Result<Arc<Self>, ProcessError> {
    ...
    ///
    /// Returns an error if the operation fails.
    pub(crate) fn open_existing(root: &Path, id: ProcessId) -> Result<Arc<Self>, ProcessError> {
        let dir = root.join(id.to_string());
        let dirfd = open_dirfd(&dir)?;
        Ok(Arc::new(Self::from_parts(id, dir, dirfd)))
    }
    ...
    #[must_use]
    pub(crate) fn dirfd_raw(&self) -> RawFd {
