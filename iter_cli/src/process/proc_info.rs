...
pub(crate) enum ProcessStartTime {
...
pub(crate) enum ProcessStartTimeParseError {
...
impl ProcessStartTime {
    ...
    #[must_use]
    pub(crate) fn to_label_string(&self) -> String {
        match self {
    ...
    pub(crate) fn from_label_string(raw: &str) -> Result<Self, ProcessStartTimeParseError> {
...
pub(crate) fn current_identity() -> Result<ProcessIdentity, ProcessError> {
...
pub(crate) fn identity_for(pid: Pid) -> Result<ProcessIdentity, ProcessError> {
...
#[cfg(target_os = "linux")]
pub(crate) fn process_start_time(pid: Pid) -> Result<ProcessStartTime, ProcessError> {
...
pub(crate) fn process_start_time(_pid: Pid) -> Result<ProcessStartTime, ProcessError> {
...
#[cfg(target_os = "linux")]
pub(crate) fn linux_boot_id() -> io::Result<&'static str> {
...
///
/// Returns an error if the operation fails.
pub(crate) fn process_is_alive_with_start_time(
    ...
    let observed = match process_start_time(identity.pid) {
        Ok(s) => s,
        // The process died between `kill -0` and `process_start_time`, or
        // `kill -0` saw a foreign-owned reused pid that this process cannot
        // introspect. iter's own runners are same-user processes, so an
        // uninspectable pid is not our live runner.
        // On Linux this surfaces as `NotFound` (/proc/<pid>/stat gone);
        // on macOS `proc_pidinfo` fails with ESRCH (raw_os_error == 3) or
        // EPERM when the pid was reused by another user's process.
        Err(ProcessError::Io(ref e)) if process_not_ours_or_vanished_io(e) => return Ok(false),
        Err(e) => return Err(e),
    };
...
fn process_not_ours_or_vanished_io(e: &io::Error) -> bool {
    ...
    #[cfg(target_os = "macos")]
    if matches!(e.raw_os_error(), Some(libc::ESRCH | libc::EPERM)) {
        return true;
    }
    false
}
...
pub(crate) fn pid_in_process_table(pid: u32) -> Result<bool, ProcessError> {
...
mod macos {
    ...
    pub(super) fn start_time(pid: Pid) -> Result<ProcessStartTime, ProcessError> {
        ...
        // SAFETY: `libc::proc_bsdinfo` is a C POD, and zeroed storage is a
        // valid initial buffer for `proc_pidinfo` to fill.
...
mod tests {
    ...
    #[test]
    fn uninspectable_pid_io_is_treated_as_not_ours() {
        let denied = io::Error::new(io::ErrorKind::PermissionDenied, "proc_pidinfo: EPERM");
        assert!(process_not_ours_or_vanished_io(&denied));
    }
