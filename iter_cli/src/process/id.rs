...
//! - [`AdoptionToken`] — 16-byte CSPRNG-generated value used to verify that
...
#[serde(transparent)]
pub(crate) struct ProcessId(Ulid);
...
impl ProcessId {
    ...
    /// Inner [`Ulid`].
    #[must_use]
    pub(crate) fn as_ulid(&self) -> Ulid {
        self.0
    }
}
...
/// 128-bit anti-accidental-adoption token written to `<dir>/adoption_token`
...
pub(crate) struct AdoptionToken([u8; 16]);
...
impl fmt::Debug for AdoptionToken {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        f.debug_struct("AdoptionToken")
            .field("len", &self.0.len())
            .finish()
    }
}

impl fmt::Display for AdoptionToken {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
mod tests {
    ...
    #[test]
    fn adoption_token_hex_is_lowercase_and_32_chars() {
        let token = AdoptionToken::from_bytes([0xab; 16]);
    ...
    #[test]
    fn adoption_token_debug_does_not_leak_payload() {
        let token = AdoptionToken::from_bytes([0xff; 16]);
