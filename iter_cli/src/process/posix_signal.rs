...
//! outside information a Runner consumes ([`iter_core::signal`]) — owns the
...
#[cfg(unix)]
pub(crate) fn send(pid: u32, kind: PosixSignal) -> ProcessFallible<()> {
    use nix::errno::Errno;
...
#[cfg(not(unix))]
pub(crate) fn send(_pid: u32, _kind: PosixSignal) -> ProcessFallible<()> {
...
pub(crate) fn signal_pid_term(pid: u32) -> ProcessFallible<()> {
...
///
/// Returns [`ProcessError::Io`] for any errno other than `ESRCH`.
pub(crate) fn signal_pid_kill(pid: u32) -> ProcessFallible<()> {
