...
//!   adds `adoption_token` before exec).
//!
...
use crate::process::lifetime_lock::LifetimeLock;
...
impl ProcessSession {
    ...
    ///   writes `adoption_token` before fork/exec; the child opens the same
    ...
    /// using the same id.
    pub(crate) async fn create_initial(
        root: &Path,
        metadata: ProcessMetadata,
        ...
        let lifetime_lock = LifetimeLock::acquire(&paths)?;
        Ok(Arc::new(Self {
            paths,
            status_file,
            metadata,
    ...
    /// validates the adoption token and runs
    /// `ProcessStatusFile::locked_adoption_write` to publish the pid file
    /// and flip `Initializing → Running`.
    pub(crate) async fn adopt(root: &Path, id: ProcessId) -> Result<Arc<Self>, ProcessError> {
    ...
    pub(crate) fn status_file(&self) -> Arc<ProcessStatusFile> {
        self.status_file.clone()
    }

    /// Frozen metadata for this session (kind, name, iterfile, args, env, …).
    pub(crate) fn metadata(&self) -> &ProcessMetadata {
        &self.metadata
    }
