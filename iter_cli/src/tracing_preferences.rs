...
pub(crate) struct TracingPreferences {
    /// Logging verbosity.
...
pub(crate) enum TracingPreferencesError {
...
impl TracingPreferences {
    ...
    pub(crate) fn load(path: Option<&Path>) -> Result<Self, TracingPreferencesError> {
...
mod tests {
    ...
    fn default_path_io_error_falls_back_to_default() {
        ...
        // SAFETY: this test mutates process-global HOME only after creating
        // its test fixture and restores the previous value before assertions.
        unsafe {
        ...
        // SAFETY: restore HOME to the exact prior process state captured
        // above.
        unsafe {
            match prev {
    ...
    fn default_path_resolves_when_home_set() {
        let prev = std::env::var_os("HOME");
        // SAFETY: this test mutates process-global HOME for one assertion and
        // restores the exact previous value before returning.
        unsafe {
